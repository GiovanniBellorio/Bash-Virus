...
# Nome File     : elimina.sh
# ---------------------------------------------------
#
# Funzionalità  :
#
# Verranno richieste le informazioni necessarie (aula,data,ora) tramite riga di comando per eliminare la prenotazione.
# In caso di prenotazione inesistente, verrà restituito un errore all'utente.
# ---------------------------------------------------
...
echo "--------------------------------------------------------------------------------"
echo "ELIMINA PRENOTAZIONE: consente di eliminare una prenotazione"
...
# Inizio modulo per eliminare una possibile prenotazione
echo "--------------------------------------------------------------------------------"
...
# Inserimento nome utente e eliminazione su file aule.txt
if [ "$flag_ora" -eq 1 ]; then
	echo -n "Inserire nome utente      : "
	read utente
	# La variabile line contiene il numero della riga all'interno del file che rappresenta la prenotazione da eliminare
	# Le informazioni non richieste vengono tagliate con il comando cut
	line=$(grep -n "$aula;$data;$ora;$utente" aule.txt | cut -d":" -f1)
	# Se line è diverso da zero procedo con l'eliminazione della riga
	if [ -n "$line" ]; then
		sed -i "/$aula;$data;$ora;$utente/d" aule.txt
		echo "Eliminazione effettuata."
	else
		echo "Aula non prenotata."
	fi
else
